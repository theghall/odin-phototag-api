...
class PhotoAPIAuthTest < ActionDispatch::IntegrationTest
  ...
  test "it should reject an API request with an empty token" do
    get api_v1_challenges_url("APITOKEN" => "")
    json_resp = JSON.parse(response.body)
    assert_equal "400", json_resp[0]["code"]
    assert_match /apitoken/i, json_resp[0]["errors"]
  end
  ...
  test "it should reject an API request with a token of invalid length" do
    get api_v1_challenges_url("APITOKEN" => "1a1a1")
    assert_response(400)
    json_resp = JSON.parse(response.body)
    assert_equal "400", json_resp[0]["code"]
  ...
  test "it should reject an API request with a token of invalid format" do
    get api_v1_challenges_url("APITOKEN" => "1a1(1a1a1a1*1a1a1;1a1a1a1a1a1a1a")
    assert_response(400)
    json_resp = JSON.parse(response.body)
    assert_equal "400", json_resp[0]["code"]
  ...
  test "it should reject an API request with an unauthorized token" do
    get api_v1_challenges_url("APITOKEN" => "1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a")
    assert_response(401)
    json_resp = JSON.parse(response.body)
